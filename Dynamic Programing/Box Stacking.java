...
class Geeks 
{
    ...
    public static int maxHeight(int height[], int width[], int length[], int n)
   {
       ...
       for(int i=0;i<n;i++){
           rot[3*i]=new Box(height[i],Math.max(width[i],length[i]),
                             Math.min(width[i],length[i]));
           
           rot[3*i+1]=new Box(width[i],Math.max(height[i],length[i]),
                            Math.min(height[i],length[i]));
                            
           rot[3*i+2]=new Box(length[i],Math.max(height[i],width[i]),
